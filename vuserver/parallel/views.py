...
from container_pool.deploy import start_consoles, stop_consoles
from script.models import Script
from script.serializers import ScriptSerializer
from script.views import updateNewScriptFile
from .consumers import send_ws_message, send_ws_status
from .models import Product, TestCase, Client, UIEvent, Console
from .serializers import ProductSerializer, TestCaseSerializer, ClientSerializer
from .serializers import UIEventSerializer, SimpleConsoleSerializer
...
class Products(APIView):
    ...
    def get(self, request, **kwargs):
        ...
        os.makedirs(settings.SCRIPTS_ROOT, exist_ok=True)
        os.makedirs(settings.CONSOLES_ROOT, exist_ok=True)
        ...
        return Response(
            {'message': 'success', 'products': products_serializer.data})

    def post(self, request, **kwargs):
        name = request.data.get('name', '')  # *
        bu_name = request.data.get('bu_name', '')  # *
        reported_issue_target = request.data.get('reported_issue_target',
                                                 'bugzilla')  # *
...
class Locales(APIView):
    ...
    def get(self, request, **kwargs):
        return Response({'message': 'success',
                         'locales': set(settings.LOCALE_MAP.values())})
...
class Resolutions(APIView):
    ...
    def get(self, request, **kwargs):
        return Response(
            {'message': 'success', 'resolutions': settings.RESOLUTIONS})
...
class Connect(APIView):
    ...
    def post(self, request):
        ...
        if not len(testcases):
            return Response(
                {'message': 'Cannot find testcase ' + testcase_uuid})
    ...
    def post(self, request):
        ...
        for recordEntry in requestData:
            ...
            try:
                ...
                current_testcase = TestCase.objects.get(
                    uuid=clients[0].testcase.uuid)
                ...
                if serializer.is_valid():
                    ...
                    if action == '':
                        if event == '19':
                            action = 'mousedown'
                            # VIU-3559
                            if temp_xpath2.find('SELECT[') > 0 and len(
                                    obj_text.split(';')) == 3:
                                action = 'select'
                        elif event == '20':
                            action = 'mouseup'
                    ...
                    send_ws_message(current_testcase.product.name,
                                    current_testcase.uuid,
                                    current_testcase.name,
                                    current_testcase.start_url, serializer.data,
                                    client_uuid,
                                    recordEntry.get('sn', 0), glossary)

                    # get report issue id and send to live console
                    if action == 'report_issue' and hasattr(settings,
                                                            "REPORTISSUE_SERVER"):
                        if settings.REPORTISSUE_SERVER:
                            reportissue_url = '{}/reportedissue/get_reported_issue_id/'.format(
                                settings.REPORTISSUE_SERVER)
                            reportissue_props = {
                                "reported_issue_utility_name": "storage",
                                "reported_issue_test_id": str(
                                    current_testcase.id),
                                "reported_issue_exec_id": str(
                                    current_testcase.run_id),
                                "reported_issue_screenshot_id": str(
                                    new_event.id),
                            ...
                            if response.status_code == requests.codes.ok:
                                reported_issue_id = response.json().get(
                                    'reported_issue_id', '')
                                logger.debug(
                                    'Got reported_issue_id: {0}'.format(
                                        reported_issue_id))
                                if reported_issue_id:
                                    send_ws_status(current_testcase.uuid,
                                                   'leader', 'en_US',
                                                   'report_issue',
                                                   reported_issue_id)
                    ...
                    logger.debug(
                        'serializer.errors: {0}'.format(serializer.errors))
                    responseMessages.append({'message': 'not valid'})
            else:
                responseMessages.append(
                    {'message': 'TestCase is not running, discard this event'})
        # end for
...
class ClientStatus(APIView):
    ...
    def post(self, request):
        ...
        if (not client_uuid) or (not clientstatus):
            ...
            client_data = {'eventid': eventid, 'status': clientstatus,
                           'message': message}
            if clientstatus == 'sendverify' and eventid != '':
                ...
                if len(events):
                    ...
                    events[0].save()
                return Response({'message': 'success'})
            send_ws_status(current_client.testcase.uuid, current_client.role,
                           current_client.locale, '', client_data)
            return Response({'message': 'success'})
...
class ShowTestcase(APIView):
    ...
    def get(self, request, testcase_uuid, format=None):
        ...
        leader_consoles = []
        if current_testcase.status == 'running':
            consoles = Console.objects.filter(appuuid=testcase_uuid,
                                              status='running')
            for c in consoles:
                if c.role == 'leader':
                    leader_consoles += [
        ...
        if current_testcase.start_url == "":
            return render(request, "autocap.html", {
                ...
                "wsserver": settings.WEBSOCKET_SERVER,
        ...
        if current_testcase.apptype == "recording":
            return render(
...
class SearchTests(APIView):
    ...
    def post(self, request):
        ...
        testcases = []
        if len(user) > 0:
            testcases = TestCase.objects.filter(softdeleted=False,
            ...
            testcases = TestCase.objects.filter(softdeleted=False,
                                                uuid__in=uuids).order_by('-id')

        pg = MyPageNumberPagination()
        page_testcases = pg.paginate_queryset(queryset=testcases,
                                              request=request, view=self)
        testcase_serializer = TestCaseSerializer(page_testcases, many=True)
        return Response(
            {'message': 'success', 'testcases': testcase_serializer.data})
...
class UserTests(APIView):
    ...
    def get(self, request, username, format=None):
        testcases = TestCase.objects.filter(user=username,
                                            softdeleted=False).order_by('-id')
        if not len(testcases):
...
class TestReport(APIView):
    ...
    def get(self, request, testcase_uuid, format=None):
        ...
        testcase_serializer = TestCaseSerializer(current_testcase)
        testruns = UIEvent.objects.filter(testcase=current_testcase). \
            values_list('run_id').distinct().order_by('-run_id')
        test_reports = {}
        for rid in [v[0] for v in testruns]:
            if rid not in test_reports:
                test_reports[rid] = {"record_time": "", "script": "",
                                     "events": "", "captures": ""}
            scripts = Script.objects.filter(test_id=current_testcase.id,
                                            run_id=rid)
            if len(scripts):
                test_reports[rid]['script'] = 'http://{}/script/download/{}'. \
                    format(settings.PARALLEL_SERVER, scripts[0].id)
            test_reports[rid]['events'] = \
                'http://{}/parallel/scriptevents/{}/{}'.\
                format(settings.PARALLEL_SERVER, current_testcase.id, rid)
            test_reports[rid]['captures'] = \
                'http://{}/parallel/captures/{}/{}'. \
                format(settings.PARALLEL_SERVER, current_testcase.id, rid)
            test_reports[rid]['video'] = 'http://{}/parallel/video/{}/{}'. \
                format(settings.PARALLEL_SERVER, current_testcase.id, rid)
            events = UIEvent.objects.filter(testcase=current_testcase,
                                            run_id=rid).order_by('recordtime')
            if len(events):
                test_reports[rid]['record_time'] = \
                    events[0].recordtime.strftime("%Y-%m-%d %H:%M:%S")

        return Response({'message': 'success',
                         'testcase': testcase_serializer.data,
                         'testruns': test_reports})
...
class TestManager(APIView):
    ...
    def get(self, request, testcase_uuid, format=None):
        if not testcase_uuid:
            ...
            pg = MyPageNumberPagination()
            page_testcases = pg.paginate_queryset(queryset=tests,
                                                  request=request, view=self)
            testcase_serializer = TestCaseSerializer(page_testcases, many=True)
            return Response(
                {'message': 'success', 'tests': testcase_serializer.data})
        elif not testcase_uuid.isdigit() and len(testcase_uuid) != 36:
            # by apptype
            tests = TestCase.objects.filter(apptype=testcase_uuid,
                                            softdeleted=False).order_by('-id')
            if not len(tests):
                ...
                page_testcases = pg.paginate_queryset(queryset=tests,
                                                      request=request,
                                                      view=self)
                testcase_serializer = TestCaseSerializer(page_testcases,
                                                         many=True)
                return Response(
                    {'message': 'success', 'tests': testcase_serializer.data})
        # test details
        ...
        testcase_serializer = TestCaseSerializer(current_testcase)
        testruns = UIEvent.objects.filter(
            testcase=current_testcase).values_list(
            'run_id').distinct().order_by('-run_id')

        consoles_data = []
        if current_testcase.status == 'running':
            running_consoles = Console.objects.filter(
                appuuid=current_testcase.uuid, status='running')
            console_serializer = SimpleConsoleSerializer(running_consoles,
                                                         many=True)
            consoles_data = console_serializer.data
        return Response(
            {'message': 'success', 'testcase': testcase_serializer.data,
             'consoles': consoles_data, 'testruns': [v[0] for v in testruns]})
    ...
    def put(self, request, testcase_uuid, format=None):
        ...
        resolution = request.data.get('resolution',
                                      current_testcase.resolution)  # *
        leader_locale = request.data.get('leader_locale',
                                         current_testcase.leader_locale)  # *
        start_url = request.data.get('start_url',
                                     current_testcase.start_url)  # *
...
class TestStart(APIView):
    ...
    def generate_default_json(self, console):
        ...
        console_folder = os.path.join(console_date, str(console.id) + '@' + str(
            console.appuuid))
        console_dir = os.path.join(settings.CONSOLES_ROOT, console_folder,
                                   'config')
        os.makedirs(console_dir, exist_ok=True)
        accessbility_enable = 'false'
        if hasattr(settings, 'EXT_ACCESSIBILITY_ENABLE'):
            accessbility_enable = settings.EXT_ACCESSIBILITY_ENABLE
        accessbility_user = ''
        if hasattr(settings, 'EXT_ACCESSIBILITY_USER'):
            accessbility_user = settings.EXT_ACCESSIBILITY_USER
        accessbility_password = ''
        if hasattr(settings, 'EXT_ACCESSIBILITY_PASSWORD'):
            accessbility_password = settings.EXT_ACCESSIBILITY_PASSWORD
        content = {
            ...
            'sendverify': 'false',
            'slavesnumber': 0,
            'prodjs': re.sub('[^a-zA-Z0-9]', '',
                             console.appproduct.lower()) + '.js',
            'extensionid': '',
            'consoleid': console.id,
            'accessibility': accessbility_enable,
            'accessibility_user': accessbility_user,
            'accessibility_password': accessbility_password,
    ...
    def generate_compose_yml(self, console):
        ...
        os.makedirs(console_dir, exist_ok=True)
        template_doc = ""
        with open(os.path.join(os.path.dirname(__file__), 'template.yml'), 'r+',
        ...
        parallel_name = "%s-%s-%s" % (console.apptype, console.id,
                                      datetime.now().strftime("%Y%m%d%H%M%S"))
        port_tag = "${PARALLEL_PORT_" + str(console.id) + "}"
        content = template_doc.format(image=settings.WORKER_IMAGE,
    ...
    def post(self, request, test_uuid, format=None):
        # pool_name = request.data.get('pool', settings.DEFAULT_POOL)  # *
        pool_name = request.data.get('pool', settings.DEFAULT_POOL)  # *
        if pool_name == "" or not pool_name:
            pool_name = settings.DEFAULT_POOL
        ...
        if not current_testcase.start_url:
            ...
            return Response(
                {'message': 'start_url is null', 'testcase': serializer.data})
        tests = TestCase.objects.filter(uuid=current_testcase.uuid,
                                        status='stopped').update(
            status='running',
            pool=pool_name,
            run_id=F('run_id') + 1,
        ...
        if tests <= 0:
            serializer = TestCaseSerializer(current_testcase)
            return Response({'message': 'Test is already running now',
                             'testcase': serializer.data})
        ...
        if current_testcase.apptype == 'parallel':
        ...
        elif current_testcase.apptype == 'recording':
            c = Console.objects.create(appname=current_testcase.name,
                                       apptype=current_testcase.apptype,
                                       appid=current_testcase.id,
                                       appuuid=current_testcase.uuid,
            ...
            return Response({'message': 'apptype not supported',
                             'apptype': current_testcase.apptype})
        ...
        for c in new_consoles:
            ...
            if f1 and f1 == f2:
                console_folders.append(
                    {'path': f1, 'id': c.id, 'apptype': c.apptype})
        response = start_consoles(console_folders, pool=pool_name)
        ...
        for c in new_consoles:
            result = deployments.get(str(c.id), {})
            if result:
                c.status = 'running'
        ...
        return Response({'message': 'success', 'testcase': serializer.data,
                         'consoles': console_serializer.data})
...
class TestStop(APIView):
    ...
    def post(self, request, test_uuid, format=None):
        ...
        running_consoles = Console.objects.filter(appuuid=current_testcase.uuid,
                                                  status='running')
        console_deployments = []
        for c in running_consoles:
            console_deployments.append(c.deployment)

        response = stop_consoles(console_deployments)
        logger.debug(
            'stopConsoles: {}, {}'.format(console_deployments, response))
        if response.get('message', '') != 'success':
            serializer = TestCaseSerializer(current_testcase)
            return Response({'message': response.get('message', ''),
                             'testcase': serializer.data})
